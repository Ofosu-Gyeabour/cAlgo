...
namespace cAlgo.Robots
{
    ...
    public class IndicesOnSteroids : Robot
    {
        ...
        [Parameter("Evaluation Period(msecs)", DefaultValue = 20000)]
...
//Percentage at which you can make input
        //[Parameter("Percentile %", DefaultValue = 60)]
        //public double Percentile { get; set; }
        private double Percentile = 60;

        [Parameter("ProfitInPips", DefaultValue = 50)]
        public double ProfitInPips { get; set; }

        [Parameter("Pips to Trigger SL", DefaultValue = 25)]
        public double PipsToTriggerSL { get; set; }

        [Parameter("Percentage Drawdown", DefaultValue = 0.015)]
        public double PercentageDrawdown { get; set; }

        [Parameter("Trailing Stop Loss", DefaultValue = 10)]
        ...
        protected override void OnStart()
        {
            ...
            int AcctNumber = 8178347;
            //edmund wayo
            //int AcctNumber = 8178288; nana akosua
            try
            {
                if (Account.IsLive)
                {
                    ...
                    strRobotName = "IndexOnSteroidsDemo";
        ...
        private void PlaceTrade(string strTradeSignal, double PIPf)
        {
            ...
            double lngVolume = 0.1;
            TradeResult result;
            //TradeOperation result;
            ...
            try
            {
                ...
                if ((pos.Length == 0))
                {
                    ...
                    if ((SymbolName == "WTI") || (SymbolName == "BRENT"))
                    {
                        lngVolume = lngVolume * 100;
                    }
                    ...
                    if ((x > 2.0) & (x < 3.0))
                    {
                        lngVolume = (2 * lngVolume) * this.scaleFactor;
                    }
...
//method spells out trading conditions or criterion to be met for a trade to be entered
        ...
        {
            //calculate potential loss in case of a losing trade
            this._Loss = (this.PercentageDrawdown * this.Account.Balance);
            Print("Account opened, potential Loss is {0}", this._Loss.ToString());
...
//this is for STOP LOSS. Take a look at the calculation again
        ...
        private bool getDrawdownPercentage(Position p)
        {
            ...
            try
            {
                ...
                var _rat = ((this.PercentageDrawdown * Account.Balance) * -1);
                this._Loss = _rat;
